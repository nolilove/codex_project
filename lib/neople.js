...
function parseNeopleDateToMs(value) {
  ...
  const fallbackMs = new Date(trimmed).getTime();
...
function normalizeTimeline(rows) {
  ...
  for (const row of rows) {
    ...
    const dateMs = parseNeopleDateToMs(date);
  ...
  timeline.sort(
    (a, b) =>
      (parseNeopleDateToMs(b.date) || Number.NEGATIVE_INFINITY) -
      (parseNeopleDateToMs(a.date) || Number.NEGATIVE_INFINITY)
  );
